...
const Profile = () => {
  ...
  useEffect(() => {
    if (user) {
      ...
      // Backend uses 'preferences' field, frontend uses 'interests'
      // preferences is an array of tag names (strings)
      const userPreferences = user.preferences || user.interests || [];
      
      // If preferences are tag names (strings), convert to tag objects
      if (userPreferences.length > 0 && typeof userPreferences[0] === 'string') {
        // Match tag names with available tags when they load
  ...
  const handleProfileSubmit = async (e) => {
    ...
    try {
      ...
      // Don't send email (backend doesn't allow email changes)
      // Don't send profile_picture if it's null
      const updateData = {
        name: profileData.name,
        username: profileData.username,
        bio: profileData.bio,
        preferences: selectedInterests.map(tag => tag.name)  // Send tag names, not IDs
      };

      // Only include profile_image_url if there's a value
      ...
      if (result.success) {
        setIsEditing(false);
        // Don't show duplicate toast - AuthContext already shows one
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-8">
          ...
          {/* Interests */}
          {((user?.interests && user.interests.length > 0) || (user?.preferences && user.preferences.length > 0)) && (
            <div className="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
              <h3 className="text-sm font-medium text-gray-900 dark:text-white mb-3">Interests</h3>
              <div className="flex flex-wrap gap-2">
                {(user.preferences || user.interests || []).slice(0, 10).map((interest, index) => {
                ...
                {(user.preferences || user.interests || []).length > 10 && (
                  <span className="text-sm text-gray-500 dark:text-gray-400">
                    +{(user.preferences || user.interests).length - 10} more
                  </span>
                )}
              </div>
            </div>
