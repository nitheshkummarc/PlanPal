...
const EventDetails = () => {
  ...
  // Unwrap event object if needed
  const event = eventRaw && eventRaw.event ? eventRaw.event : eventRaw;
  ...
  const loadParticipationStatus = async () => {
    if (!user) {
      // If user is not authenticated, skip participation status check
  ...
  const handleJoinEvent = async () => {
    ...
    try {
      ...
      // Refresh event details and participation status
      await Promise.all([
        fetchEventRaw(eventId),
        loadParticipationStatus()
      ]);
      
      // Add small delay to ensure backend has processed the join
      setTimeout(() => {
        // Trigger calendar refresh with multiple methods for reliability
  ...
  // Use posted_by for creator check (backend returns posted_by as user_id)
  const isOwner = user?.user_id === event.posted_by || user?.user_id === event.created_by?.user_id;
  // Participation status logic (backend returns status: 'going', 'interested', or 'not_joined')
  ...
  const canEdit = user && isOwner; // Only event owners can edit
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
          <div className="p-6">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-gray-900 dark:text-white font-medium">
                      {event.timestamp ? formatDate(new Date(event.timestamp)) : 'N/A'}
                    </p>
                  </div>
                </div>
            ...
            <div className="flex items-center justify-between pt-6 border-t border-gray-200 dark:border-gray-700">
              ...
              <div className="flex items-center gap-3">
                {/* Leave Event Button - for participants only */}
                {canLeave && (
                  <LoadingButton
                ...
                {/* Join Event Button - for non-participants, non-owners on upcoming events */}
                {canJoin && (
                  <LoadingButton
                ...
                {/* Already Joined Status - for participants */}
                ...
                {/* Edit Event Button - only for event organizers */}
                {canEdit && (
                  <button
