...
const Search = () => {
  ...
  useEffect(() => {
    // Trigger search when query exists OR when tags are selected (allows tag-only search)
    if (debouncedQuery.trim() || selectedTags.length > 0) {
      handleSearch();
    } else {
      // Clear results when no query and no tags
      resetSearchResults();
    }
  ...
  const handleSearch = async () => {
    try {
      ...
      // Allow search with empty query if tags are selected
